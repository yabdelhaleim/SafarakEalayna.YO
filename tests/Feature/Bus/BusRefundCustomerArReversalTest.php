...
class BusRefundCustomerArReversalTest extends BusTestCase
{
    ...
    /**
     * Multi-currency refund scenario — USD booking + EGP cashbox.
     *
     * Setup:
     *   - USD-priced inventory ($3 sell, $2 cost, 1 USD = 50 EGP)
     *   - Customer pays in USD wallet
     *
     * After booking + pay:
     *   - Customer USD AR = $6 (cleared to 0 by payment Transfer)
     *   - Wallet USD = +$6
     *   - Supplier EGP AP = -200 (cost $4 × 50 = 200 EGP)
     *
     * Refund scenario: refund full $6 back to USD treasury.
     *
     * Phase 9 fix: customer USD AR must reverse by -$6 (-$6 USD balance
     * = office owes customer back in USD).
     */
    public function test_refund_reverses_customer_ar_in_usd_lifecycle(): void
    {
        $company = $this->makeBusCompany([], 0);
        $inventory = $this->makeInventory([
            'company_id' => $company->id,
        ...
        $service->createBooking([
            'inventory_id' => $inventory->id,
            'customer_name' => 'USD Refund AR Test',
            'customer_phone' => '010R0000002',
            'quantity' => 2, // 2 × $3 = $6
        ]);

        $booking = BusBooking::latest('id')->firstOrFail();
        $customer = Customer::where('phone', '010R0000002')->firstOrFail();
        $customerAccount = Account::find($customer->account_id);

        // Pay in USD via the seeded USD wallet
        $this->postJson("/api/v1/bus/bookings/{$booking->id}/pay", [
            'amount' => 6.0,
        ...
        $this->assertEqualsWithDelta(
            ...
            (float) $customerAccount->fresh()->balance,
            0.01,
            'After USD payment, customer USD AR must be 0'
        );

        // Create USD treasury + refund request
        $treasury = Treasury::query()->create([
            'name' => 'USD Treasury',
            'currency' => 'USD',
            'current_balance' => 0,
        ...
        $refundRequest = BusRefundRequest::findOrFail($refundId);
        $this->assertEquals(6.0, (float) $refundRequest->refund_amount);

        // Process refund
        $this->postJson("/api/v1/bus/refunds/{$refundId}/process")->assertOk();

        // Customer USD AR must be -6.0 (office owes customer back in USD)
        $this->assertEqualsWithDelta(
            -6.0,
            (float) $customerAccount->fresh()->balance,
            0.01,
            'After USD refund processing, customer USD AR must be -$6 (office owes customer)'
        );

        // Treasury USD credited by 6
        $this->assertEqualsWithDelta(
            6.0,
            (float) $treasury->fresh()->current_balance,
            0.01,
            'USD treasury must be credited by $6'
        );

        // Supplier EGP AP reversed from -200 to 0
        $this->assertEqualsWithDelta(
            0.0,
            (float) $company->account->fresh()->balance,
            0.01,
            'Supplier EGP AP must be cleared (reversed from -200)'
        );
