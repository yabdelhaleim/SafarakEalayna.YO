      ...
      <form @submit.prevent="saveProgram" class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input v-model.number="form.mecca_nights" type="number" required class="w-full p-4 bg-input border border-white/10 rounded-xl focus:border-gold outline-none" />
          </div>
          ...
          <div>
            ...
            <input v-model.number="form.medina_nights" type="number" class="w-full p-4 bg-input border border-white/10 rounded-xl focus:border-gold outline-none" />
          </div>
          ...
          <div class="md:col-span-2">
            <div class="flex gap-4">
              <label class="flex items-center gap-3 p-4 bg-input border border-white/10 rounded-xl cursor-pointer hover:border-gold/50 transition-all flex-1">
                <input v-model="form.booking_status" type="radio" value="active" class="w-5 h-5" />
                <div>
                  <div class="font-bold">نشط</div>
                  <div class="text-sm text-muted">متاح للحجز</div>
                </div>
              </label>
              <label class="flex items-center gap-3 p-4 bg-input border border-white/10 rounded-xl cursor-pointer hover:border-gold/50 transition-all flex-1">
                <input v-model="form.booking_status" type="radio" value="closed" class="w-5 h-5" />
                <div>
                  <div class="font-bold">مغلق</div>
                  <div class="text-sm text-muted">غير متاح</div>
                </div>
              </label>
            </div>
          </div>
...
const saveProgram = async () => {
  isSaving.value = true;
  try {
    await axios.post('/api/v1/programs', form.value);
