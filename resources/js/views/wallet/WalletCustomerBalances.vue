  ...
  <div class="space-y-8 animate-in fade-in duration-700" dir="rtl">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl sm:text-3xl md:text-4xl font-extrabold text-text-main tracking-tight">
          مديونيات العملاء (المحافظ والتحويلات)
        </h1>
        <p class="text-text-muted mt-1">
          متابعة وتدقيق مديونيات وأرصدة العملاء في موديول المحافظ والتحويلات
        </p>
      </div>
    ...
    <div class="bg-card-bg border border-white/10 rounded-2xl p-6">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div class="md:col-span-2 lg:col-span-1">
          ...
          <div class="relative">
            ...
            <input
              ...
              class="w-full pr-10 pl-4 py-2.5 bg-white/5 border border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple/50 focus:border-purple transition-all"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <select v-model="filters.wallet_type_id" @change="fetchBalances" class="form-select-dark py-2.5 text-sm">
            <option value="all">الكل</option>
        ...
        <div>
          ...
          <input
            ...
            class="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple/50 focus:border-purple transition-all text-sm"
          />
        </div>
    ...
    <div class="bg-card-bg border border-white/10 rounded-2xl overflow-hidden shadow-xl">
      <div v-if="loading" class="flex flex-col items-center justify-center py-20">
        <Loader2 class="w-10 h-10 animate-spin text-purple mb-4" />
            ...
            >
              ...
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-purple/10 text-sm font-black text-purple border border-purple/20">
              ...
              <td class="px-6 py-4 text-left">
                <div class="flex gap-2 justify-end">
                  ...
                  <button
                    ...
                    class="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-semibold text-white/60 transition hover:border-purple/40 hover:bg-purple/10 hover:text-purple"
                  >
                    ...
                    <td class="px-4 py-3">
                      <span
                        :class="[
                          ...
                          tx.type === 'عملية' ? 'bg-purple/10 text-purple' : 'bg-success/10 text-success'
                        ]"
                      >
                        {{ tx.type }}
                      </span>
    ...
    <div
      v-if="printCustomer"
      id="wallet-statement-print-content"
      dir="rtl"
    ...
    >
      ...
      <div class="flex items-center justify-between border-b border-gray-300 pb-4 mb-6">
        <div>
          ...
          <p class="text-xs text-gray-500 mt-1">كشف حساب عميل - موديول المحافظ والتحويلات</p>
        </div>
        <div class="text-left">
        ...
        <tbody>
          <tr v-for="tx in printTransactions" :key="tx.id" class="hover:bg-gray-50">
            ...
            <td class="border border-gray-300 px-3 py-2">
              <span :class="tx.type === 'عملية' ? 'text-purple-600 font-bold' : 'text-emerald-600 font-bold'">
                {{ tx.type }}
              </span>
            </td>
...
// Wallet types
const walletTypes = computed(() => store.walletTypes || []);
...
const customerStats = computed(() => {
  ...
  return [
    ...
    },
    {
      ...
      icon: ListOrdered, iconBg: 'bg-purple-500/15', iconColor: 'text-purple-400',
      valueColor: 'text-purple-400', glow: 'bg-purple-400',
    },
    {
...
const fetchBalances = async () => {
  ...
  currentPage.value = 1;
  try {
    const res = await axios.get('/api/v1/wallet/customer-balances', {
      params: {
        search: filters.value.search,
        status: filters.value.status,
        wallet_type_id: filters.value.wallet_type_id,
...
const openDetails = async (row) => {
  ...
  try {
    const res = await axios.get('/api/v1/wallet/customer-statement', {
      params: {
...
const printStatement = async (row) => {
  ...
  try {
    ...
    document.documentElement.classList.add('wallet-print-active');
    document.title = `كشف حساب محفظة — ${row.client_name}`;

    const cleanup = () => {
      document.documentElement.classList.remove('wallet-print-active');
...
const exportCsv = () => {
  ...
  link.setAttribute('download', `wallet_debts_${new Date().toISOString().slice(0, 10)}.csv`);
...
@media print {
  ...
  html.wallet-print-active {
    background: #ffffff !important;
    color: #000000 !important;
  }

  /* Hide everything except the print-content block when active */
  html.wallet-print-active body * {
    visibility: hidden !important;
  }

  html.wallet-print-active #wallet-statement-print-content,
  html.wallet-print-active #wallet-statement-print-content * {
  ...
  #wallet-statement-print-content table {
    width: 100% !important;
  ...
  #wallet-statement-print-content tr {
    page-break-inside: avoid !important;
    break-inside: avoid !important;
  }

  #wallet-statement-print-content th, 
  #wallet-statement-print-content td {
