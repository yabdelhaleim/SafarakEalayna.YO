  ...
  <div class="animate-in fade-in duration-500 pb-16">
    ...
    <header class="relative overflow-hidden bg-gradient-to-br from-[#0a1628] via-[#0d1f3c] to-[#111827] border-b border-white/5">
      <div class="absolute inset-0 pointer-events-none">
        ...
        <div class="absolute bottom-0 left-0 w-64 h-64 bg-cyan-500/10 rounded-full blur-3xl translate-y-1/2 -translate-x-1/3"></div>
      </div>
          ...
          <div>
            <div class="flex items-center gap-3 mb-3">
              <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-500/20 border border-blue-500/30">
                <Bus class="h-5 w-5 text-blue-400" />
              </div>
              <span class="text-xs font-bold uppercase tracking-[0.2em] text-blue-400/80">وحدة الباص</span>
            </div>
            ...
            <p class="mt-2 max-w-2xl text-sm leading-relaxed text-white/50">
              نظرة عامة على نشاط قسم الباصات وحركة الأموال — إيرادات، حجوزات، وأرصدة الحسابات والمديونيات.
            </p>
          </div>
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 mt-8 space-y-8">
      ...
      <template v-else-if="data">
        ...
        <section class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <!-- Company Debt -->
          <div class="group relative overflow-hidden rounded-2xl border border-red-500/20 bg-gradient-to-br from-red-500/10 to-transparent p-6 transition hover:border-red-500/40 hover:shadow-lg hover:shadow-red-500/10">
            ...
            <div class="relative">
              ...
              <p class="text-xs font-bold uppercase tracking-wider text-red-400/70 mb-1">مديونيات الشركات</p>
              <p class="font-mono text-2xl font-black text-white tabular-nums">
                {{ fmt(data.total_company_debt) }}
              </p>
              <p class="text-[11px] text-white/30 mt-1">جنيه مصري</p>
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          <div class="lg:col-span-2 space-y-4">
            <div class="flex items-center justify-between">
              ...
              <router-link :to="{ name: 'bus.list' }" class="text-xs font-bold text-blue-400 hover:text-blue-300 transition flex items-center gap-1">
                عرض الكل
              ...
              <table class="min-w-full text-right text-sm">
                ...
                <tbody class="divide-y divide-white/5">
                  <tr
                    ...
                    @click="$router.push({ name: 'bus.show', params: { id: booking.id } })"
                  >
                    <td class="px-5 py-3.5">
                      <span class="font-mono font-bold text-blue-400 text-xs">{{ booking.booking_number }}</span>
                    </td>
                    <td class="px-5 py-3.5 font-medium text-white/90 text-sm">{{ booking.customer?.name || '—' }}</td>
                    <td class="px-5 py-3.5">
                      <span v-if="booking.route" class="font-mono text-xs text-white/60">
                        {{ booking.route }}
                      </span>
                    ...
                    <td class="px-5 py-3.5">
                      <span :class="['px-2 py-0.5 rounded-full text-[10px] font-bold uppercase', getStatusClass(booking.status)]">
                        {{ booking.status }}
                      </span>
                    </td>
          ...
          <div class="space-y-6">
            ...
            <div class="rounded-2xl border border-blue-500/20 bg-gradient-to-b from-blue-950/60 to-transparent p-6 text-center relative overflow-hidden">
              ...
              <div class="relative">
                ...
                <p class="text-xs text-white/30">يشمل جميع حسابات الباصات</p>
              </div>
            </div>
            ...
            <div class="rounded-2xl border border-white/5 bg-white/[0.02] p-5">
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <span class="text-xs font-bold text-blue-200">حجز جديد</span>
                </router-link>
                <router-link
                  :to="{ name: 'bus.list' }"
                ...
                >
                  <Building2 class="h-6 w-6 text-red-400 transition group-hover:scale-110" />
                  <span class="text-xs font-bold text-red-200">الشركات</span>
                </router-link>
              </div>
            </div>
...
const reload = async () => {
  loading.value = true;
  data.value = await store.fetchBusDashboard();
  loading.value = false;
