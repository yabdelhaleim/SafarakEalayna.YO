...
const fetchStats = async () => {
  try {
    ...
    // Total debt — sum customer.balance for customers with flight bookings
    // (the pre-89f6dff behaviour; reverts the broken flight_remaining_debt
    // aggregate which referenced non-existent flight_bookings columns).
    ...
    let debtSum = 0;
    items.forEach(c => {
      const d = parseFloat(c.balance || 0);
      if (d > 0) debtSum += d;
    });
...
const openPayDebtModal = (customerRef, booking = null) => {
  ...
  if (isGroup) {
    ...
    // Reverts Phase 4: use customer.balance directly. flight_remaining_debt
    // is always 0 because its backing withSum referenced non-existent
    // flight_bookings columns (total_price / paid_amount).
    // balance > 0 = receivable (سند قبض)
    // balance < 0 = we owe customer (سند صرف)
  ...
  if (booking) {
    ...
    // Pre-fill with the customer's ledger balance.
    const customerDebt = parseFloat(customer.balance || 0);
    payDebtForm.amount = customerDebt !== 0 ? Math.abs(customerDebt) : '';
