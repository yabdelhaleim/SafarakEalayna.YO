<template>
  <div class="finance-dashboard fawry-module animate-in pb-10 fade-in duration-700">
    <header class="fawry-hero relative overflow-hidden bg-gradient-to-br from-[#0a1628] via-[#0d1f3c] to-[#111827] border-b border-white/5 py-10 px-4 sm:px-6 lg:px-8">
      <div class="relative z-10 mx-auto flex max-w-7xl flex-col gap-4 lg:flex-row lg:items-end lg:justify-between lg:px-8">
        <div class="min-w-0 flex-1">
          <div class="flex items-center gap-3 mb-2">
            <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-500/20 border border-emerald-500/30">
              <Vault class="h-4 w-4 text-emerald-400" />
            </div>
            <p class="text-[11px] font-bold uppercase tracking-[0.2em] text-emerald-400/90">خزينة فوري</p>
          </div>
          <h1 class="text-3xl font-black tracking-tight text-white sm:text-4xl">
            أرصدة التحصيل والحسابات
          </h1>
          <p class="mt-2 max-w-2xl text-sm leading-relaxed text-white/50">
            متابعة حسابات موديول فوري (محافظ، بنوك، بريد، خزائن نقدي) وإجمالي السيولة المتاحة.
          </p>
        </div>
        <div class="flex shrink-0 flex-wrap gap-2">
          <router-link
            :to="{ name: 'fawry.index' }"
            class="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-bold text-white/70 transition hover:border-emerald-500/40 hover:text-white"
          >
          ...
          <button
            type="button"
            class="inline-flex items-center gap-2 rounded-xl border border-emerald-500/30 bg-emerald-500/15 px-4 py-2 text-sm font-bold text-emerald-200 transition hover:bg-emerald-500/25"
            @click="reload"
          >
    ...
    <div class="mx-auto max-w-7xl space-y-10 px-4 sm:px-6 lg:px-8 mt-8">
      ...
      <template v-else-if="ov">
        ...
        <section class="space-y-8">
          ...
          <!-- Treasuries -->
          <div v-if="ov.treasuries?.length" class="space-y-4">
            <h2 class="text-xl font-bold text-white flex items-center gap-2">
              <Briefcase class="w-5 h-5 text-rose-400" />
              الخزائن العامة
            </h2>
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
              <div v-for="acc in ov.treasuries" :key="acc.id" class="rounded-2xl border border-rose-500/20 bg-gradient-to-br from-rose-500/10 to-transparent p-6 group transition hover:border-rose-500/40">
                <div class="flex items-center justify-between mb-4">
          ...
          <div class="overflow-x-auto rounded-2xl border border-white/5 bg-white/[0.02]">
            <table class="min-w-full text-right text-sm">
              ...
              <tbody class="divide-y divide-white/5">
                <tr
                  v-for="tx in ov.recent_transactions || []"
                  :key="tx.id"
                ...
                <tr v-if="!ov.recent_transactions?.length">
      ...
      >
        <div class="max-h-[85vh] w-full max-w-4xl overflow-hidden rounded-2xl border border-white/10 bg-[#0a111e] shadow-2xl animate-in zoom-in-95 duration-200">
          <div class="flex items-center justify-between border-b border-white/5 px-6 py-5">
            <div>
              <p class="text-[10px] font-bold uppercase tracking-widest text-white/40">حركات موديول فوري على الحساب</p>
          ...
          <div class="max-h-[60vh] overflow-auto p-6">
            ...
            <div v-if="!accountTxLoading && !accountTxRows.length" class="py-12 text-center text-white/20">لا توجد حركات مسجلة لقسم فوري على هذا الحساب.</div>
          </div>
...
import { ArrowRight, RefreshCw, Vault, Smartphone, Landmark, Banknote, Briefcase, Clock } from 'lucide-vue-next';
...
const reload = async () => {
  loading.value = true;
  try {
    await store.fetchFawryTreasuryOverview();
  } finally {
    loading.value = false;
  }
};
...
const loadAccountPage = async (page) => {
  ...
  try {
    const data = await store.fetchAccountFawryTransactions(modal.value.account.id, { page, per_page: 25 });
...
<style scoped>
.fawry-hero {
  background-image: radial-gradient(circle at 10% 20%, rgba(16, 185, 129, 0.05) 0%, transparent 50%);
}
</style>
