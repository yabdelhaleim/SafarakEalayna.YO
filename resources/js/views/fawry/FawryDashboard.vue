  ...
  <div class="animate-in fade-in duration-500 pb-16 fawry-module">
        ...
        <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p class="mt-2 max-w-2xl text-sm leading-relaxed text-white/50">
              إحصائيات المعاملات، الأرباح اليومية، والسيولة المتاحة في الحسابات التابعة لموديول فوري.
            </p>
          </div>
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 mt-8 space-y-8">
      ...
      <template v-else-if="data">
        <!-- KPI Cards -->
        <section :class="['grid gap-5', isAdmin ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-4' : 'grid-cols-1 sm:grid-cols-2']">
          <!-- Today Revenue -->
          <div v-if="isAdmin" class="group relative overflow-hidden rounded-2xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/10 to-transparent p-6 transition hover:border-emerald-500/40 hover:shadow-lg hover:shadow-emerald-500/10">
            ...
            <div class="relative">
              ...
              <p class="text-xs font-bold uppercase tracking-wider text-emerald-400/70 mb-1">إيرادات اليوم</p>
              <p class="font-mono text-2xl font-black text-white tabular-nums">
                {{ fmt(data.stats.today_revenue) }}
              </p>
          ...
          <!-- Monthly Profit -->
          <div v-if="isAdmin" class="group relative overflow-hidden rounded-2xl border border-indigo-500/20 bg-gradient-to-br from-indigo-500/10 to-transparent p-6 transition hover:border-indigo-500/40 hover:shadow-lg hover:shadow-indigo-500/10">
            ...
            <div class="relative">
              <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-[10px] font-bold uppercase tracking-wider bg-indigo-500/20 text-indigo-400 px-2 py-0.5 rounded-full">الشهر</span>
              </div>
              <p class="text-xs font-bold uppercase tracking-wider text-indigo-400/70 mb-1">صافي الربح</p>
              <p class="font-mono text-2xl font-black text-white tabular-nums">
                {{ fmt(data.stats.monthly_profit) }}
              </p>
              <p class="text-[11px] text-white/30 mt-1">جنيه مصري</p>
            </div>
          </div>
          ...
          <div class="group relative overflow-hidden rounded-2xl border border-sky-500/20 bg-gradient-to-br from-sky-500/10 to-transparent p-6 transition hover:border-sky-500/40 hover:shadow-lg hover:shadow-sky-500/10">
            ...
            <div class="relative">
              ...
              <p class="text-xs font-bold uppercase tracking-wider text-sky-400/70 mb-1">إجمالي العمليات</p>
              <p class="font-mono text-2xl font-black text-white tabular-nums">
          ...
          <div v-if="isAdmin" class="group relative overflow-hidden rounded-2xl border border-amber-500/20 bg-gradient-to-br from-amber-500/10 to-transparent p-6 transition hover:border-amber-500/40 hover:shadow-lg hover:shadow-amber-500/10">
            ...
            <div class="relative">
              ...
              <p class="text-xs font-bold uppercase tracking-wider text-amber-400/70 mb-1">إجمالي السيولة</p>
              <p class="font-mono text-2xl font-black text-white tabular-nums">
              ...
              <table class="min-w-full text-right text-sm">
                ...
                <tbody class="divide-y divide-white/5">
                  ...
                  >
                    ...
                    <td v-if="isAdmin" class="px-5 py-3.5 font-mono font-bold text-emerald-400 text-sm">
                      +{{ fmt(tx.profit) }}
                    </td>
          ...
          <div class="space-y-6">
            ...
            <div v-if="isAdmin" class="rounded-2xl border border-white/5 bg-white/[0.02] p-5 space-y-3">
              ...
              <div class="flex items-center justify-between py-2 border-b border-white/5">
                ...
                <span class="font-mono font-bold text-sm text-emerald-400">{{ fmt(data.stats.wallets.balance) }}</span>
              </div>
              <div class="flex items-center justify-between py-2 border-b border-white/5">
                <div class="flex items-center gap-2">
      ...
      <div v-else class="flex flex-col items-center justify-center py-32 gap-5">
        ...
        <div class="text-center">
          ...
          <p class="text-sm text-white/40 mt-1">فشل جلب إحصائيات داش بورد فوري</p>
        </div>
