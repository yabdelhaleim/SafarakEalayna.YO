<template>
  <div class="space-y-8 animate-in fade-in duration-700" dir="rtl">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl sm:text-3xl md:text-4xl font-extrabold text-text-main tracking-tight">
          مديونيات العملاء (فوري)
        </h1>
        <p class="text-text-muted mt-1">
          متابعة وتدقيق مديونيات وأرصدة العملاء في موديول فوري
        </p>
      </div>
      <div class="flex gap-3">
        <button
          @click="exportCsv"
          class="px-6 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl font-semibold flex items-center justify-center gap-2 transition-all text-sm"
        >
    ...
    <!-- Stats row (glowing cards styled like Bus module) -->
    ...
    <div class="bg-card-bg border border-white/10 rounded-2xl p-6">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <!-- Machine Filter -->
        <div>
          <label class="block text-sm font-medium text-text-muted mb-2">الماكينة / الحساب</label>
          <select v-model="filters.machine" @change="fetchBalances" class="form-select-dark py-2.5 text-sm">
            <option value="all">الكل</option>
            <option value="فوري">ماكينات فوري</option>
            <option value="أوباي">ماكينات أوباي</option>
            <option value="أمان">ماكينات أمان</option>
          </select>
        </div>
    ...
    <div class="bg-card-bg border border-white/10 rounded-2xl overflow-hidden shadow-xl">
      ...
      <div v-else-if="records.length === 0" class="text-center py-20">
        ...
        <h3 class="text-xl font-bold text-text-main">لا توجد مديونيات مسجلة</h3>
              ...
              <td class="px-6 py-4 text-left">
                <div class="flex gap-2 justify-end">
                  ...
                  <button
                    ...
                    class="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-semibold text-white/60 transition hover:border-gold/40 hover:bg-gold/10 hover:text-gold"
                  >
    ...
    >
      <div class="bg-card-bg border border-white/15 w-full max-w-4xl rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh] animate-in zoom-in duration-300">
        ...
        <div class="p-6 border-b border-white/10 flex items-center justify-between bg-white/[0.02]">
          <div>
            <h3 class="text-xl font-bold text-text-main">{{ selectedCustomerName }}</h3>
          ...
          <button @click="modalOpen = false" class="p-2 hover:bg-white/10 rounded-lg transition-colors">
            <X class="w-6 h-6 text-text-muted hover:text-text-main" />
          </button>
        </div>
        ...
        <div class="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          <div class="flex justify-between items-center bg-white/[0.03] border border-white/10 p-5 rounded-xl">
            <div>
              ...
              <span class="text-2xl font-black mt-1 inline-block" :class="selectedCustomerRunningBalance > 0 ? 'text-error' : (selectedCustomerRunningBalance < 0 ? 'text-success' : 'text-text-muted')">
                {{ formatMoney(Math.abs(selectedCustomerRunningBalance)) }}
                <span class="text-xs font-normal">({{ selectedCustomerRunningBalance > 0 ? 'مستحق عليه / مدين' : (selectedCustomerRunningBalance < 0 ? 'له طرفنا / دائن' : 'صفر') }})</span>
              </span>
            </div>
            <button
              ...
              <table class="w-full text-right text-sm">
                <thead class="bg-white/5 border-b border-white/10">
                  <tr>
                    ...
                    <th class="px-4 py-3 text-text-muted">الماكينة / الحساب</th>
                ...
                <tbody class="divide-y divide-white/5">
                  <tr v-for="tx in modalTransactions" :key="tx.id" class="hover:bg-white/[0.02]">
                    <td class="px-4 py-3 font-medium text-text-main">{{ tx.date }}</td>
                    <td class="px-4 py-3">{{ tx.machine }}</td>
                    <td class="px-4 py-3">
                      <span
                        :class="[
                          'px-2 py-0.5 rounded text-xs font-bold',
                          tx.type === 'سحب' ? 'bg-error/10 text-error' : 'bg-success/10 text-success'
                        ]"
                      >
                        {{ tx.type }}
                      </span>
                    ...
                    <td class="px-4 py-3 font-mono font-semibold" :class="tx.running_balance > 0 ? 'text-error' : (tx.running_balance < 0 ? 'text-success' : 'text-text-muted')">
          ...
          <form @submit.prevent="submitPayment" class="space-y-5 p-6">
            ...
            <div class="space-y-2">
              <div class="flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3">
                <div class="flex items-center gap-3">
                  <div class="flex h-9 w-9 items-center justify-center rounded-xl bg-red-500/15 text-sm font-black text-red-400">
                ...
                <div class="text-left">
                  ...
                  <p class="font-mono text-sm font-bold text-gold">
              ...
              <div class="flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3">
                <span class="text-sm text-text-muted">إجمالي المبيعات</span>
    ...
    <div
      v-if="printCustomer"
      id="fawry-statement-print-content"
      dir="rtl"
    ...
    >
      ...
      <div class="flex items-center justify-between border-b border-gray-300 pb-4 mb-6">
        <div>
          ...
          <p class="text-xs text-gray-500 mt-1">كشف حساب عميل - موديول فوري</p>
        </div>
        <div class="text-left">
      ...
      <table class="w-full border-collapse border border-gray-300 mb-8 text-sm">
        <thead>
          <tr class="bg-gray-100">
            ...
            <th class="border border-gray-300 px-3 py-2 text-right">الماكينة / الحساب</th>
        ...
        <tbody>
          <tr v-for="tx in printTransactions" :key="tx.id" class="hover:bg-gray-50">
            ...
            <td class="border border-gray-300 px-3 py-2">
              <span :class="tx.type === 'سحب' ? 'text-red-600 font-bold' : 'text-emerald-600 font-bold'">
                {{ tx.type }}
              </span>
            </td>
...
const filteredAccounts = computed(() => {
  if (settlementCategoryUi.value === 'cash') {
    return fawryAccounts.value.filter(a => a.type === 'cashbox' || a.type === 'treasury');
  }
  if (settlementCategoryUi.value === 'wallet') {
    return fawryAccounts.value.filter(a => a.type === 'wallet');
  }
  if (settlementCategoryUi.value === 'bank') {
    return fawryAccounts.value.filter(a => a.type === 'bank');
  }
  return fawryAccounts.value;
});
...
// Stats cards (Amber theme consistent with Fawry)
const customerStats = computed(() => {
  ...
  return [
    ...
    },
    {
      ...
      icon: ListOrdered, iconBg: 'bg-gold/15', iconColor: 'text-gold',
      valueColor: 'text-gold', glow: 'bg-gold',
    },
    {
...
const fetchBalances = async () => {
  ...
  currentPage.value = 1;
  try {
    const res = await axios.get('/api/v1/fawry/customer-balances', {
      params: {
        search: filters.value.search,
        status: filters.value.status,
        machine: filters.value.machine,
...
const openDetails = async (row) => {
  ...
  try {
    const res = await axios.get('/api/v1/fawry/customer-statement', {
      params: {
...
const printStatement = async (row) => {
  ...
  try {
    ...
    document.documentElement.classList.add('fawry-print-active');
    document.title = `كشف حساب فوري — ${row.client_name}`;

    const cleanup = () => {
      document.documentElement.classList.remove('fawry-print-active');
    ...
    store.addToast('فشل تحميل كشف الحساب للطباعة', 'error');
  } finally {
    loading.value = false;
  }
};
...
const exportCsv = () => {
  ...
  csvContent += 'اسم العميل,رقم الهاتف,إجمالي المبيعات (ج.م),المدفوع (ج.م),الآجل المتبقي (ج.م),عدد المعاملات,آخر معاملة\r\n';
  ...
  link.setAttribute('download', `fawry_debts_${new Date().toISOString().slice(0, 10)}.csv`);
...
.form-select-dark:focus {
  ...
  box-shadow: 0 0 0 2px var(--blue-lo);
}
</style>
...
@media print {
  ...
  html.fawry-print-active {
    background: #ffffff !important;
    color: #000000 !important;
  }

  /* Hide everything except the print-content block when active */
  html.fawry-print-active body * {
    visibility: hidden !important;
  }

  html.fawry-print-active #fawry-statement-print-content,
  html.fawry-print-active #fawry-statement-print-content * {
  ...
  #fawry-statement-print-content table {
    width: 100% !important;
  ...
  #fawry-statement-print-content tr {
    page-break-inside: avoid !important;
    break-inside: avoid !important;
  }

  #fawry-statement-print-content th, 
  #fawry-statement-print-content td {
