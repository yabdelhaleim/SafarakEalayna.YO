<template>
  <div class="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-700">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-extrabold text-white">تعديل طلب التأشيرة</h1>
        <p class="text-muted mt-1">طلب #{{ booking?.id }}</p>
      </div>
      <router-link :to="{ name: 'visa.show', params: { id: booking?.id } }" class="text-muted hover:text-white flex items-center gap-2 transition-colors">
    ...
    <div v-else-if="booking" class="space-y-6">
      ...
      <div class="p-6 bg-card border border-white/10 rounded-2xl space-y-6">
        <h2 class="text-lg font-bold">تحديث الطلب</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label class="block text-xs text-muted mb-2">اسم الموظف</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label class="block text-xs text-muted mb-2">رسوم الخدمة</label>
            <input v-model.number="form.service_fee" type="number" min="0" step="0.01" class="w-full p-4 bg-input border border-white/10 rounded-xl focus:border-gold outline-none font-mono" />
          </div>
        </div>
        ...
        <div class="p-4 bg-input/40 border border-white/10 rounded-xl space-y-3">
          ...
          <div class="flex flex-wrap gap-2">
            <button v-for="p in [20, 30, 50, 100]" :key="p"
              @click="applyMarkup(p)" type="button"
        ...
        <!-- Visa details edits -->
        <h3 class="text-md font-bold mt-2">تفاصيل التأشيرة</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label class="block text-xs text-muted mb-2">نوع التأشيرة</label>
            <select v-model="form.visa_details.visa_type" class="w-full p-4 bg-input border border-white/10 rounded-xl focus:border-gold outline-none">
          ...
          <div>
            ...
            <input v-model="form.visa_details.country" type="text" class="w-full p-4 bg-input border border-white/10 rounded-xl focus:border-gold outline-none" />
          </div>
          <div>
            <label class="block text-xs text-muted mb-2">المدّة</label>
            <select v-model="form.visa_details.visa_duration_id" class="w-full p-4 bg-input border border-white/10 rounded-xl focus:border-gold outline-none">
              <option :value="null">— اختر —</option>
              <option v-for="d in store.durations" :key="d.id" :value="d.id">{{ d.label }}</option>
            </select>
          </div>
          <div>
            <label class="block text-xs text-muted mb-2">نوع الدخول</label>
            <select v-model="form.visa_details.entry_type" class="w-full p-4 bg-input border border-white/10 rounded-xl focus:border-gold outline-none">
              <option v-for="e in (store.statuses?.visa_entry_types || [])" :key="e.value" :value="e.value">{{ e.label }}</option>
            </select>
          </div>
          <div>
            <label class="block text-xs text-muted mb-2">الوكيل</label>
            <select v-model="form.visa_details.visa_agent_id" class="w-full p-4 bg-input border border-white/10 rounded-xl focus:border-gold outline-none">
              <option :value="null">— اختر —</option>
              <option v-for="a in store.agents" :key="a.id" :value="a.id">{{ a.name }}</option>
            </select>
          </div>
          <div>
            <label class="block text-xs text-muted mb-2">رقم التأشيرة</label>
            <input v-model="form.visa_details.visa_number" type="text" class="w-full p-4 bg-input border border-white/10 rounded-xl focus:border-gold outline-none font-mono" />
          </div>
          <div>
            <label class="block text-xs text-muted mb-2">صلاحية من</label>
            <input v-model="form.visa_details.validity_from" type="date" class="w-full p-4 bg-input border border-white/10 rounded-xl focus:border-gold outline-none" />
          </div>
          <div>
            <label class="block text-xs text-muted mb-2">صلاحية إلى</label>
            <input v-model="form.visa_details.validity_to" type="date" class="w-full p-4 bg-input border border-white/10 rounded-xl focus:border-gold outline-none" />
          </div>
        </div>
        ...
        <div class="flex gap-3">
          ...
          <router-link :to="{ name: 'visa.show', params: { id: booking.id } }" class="flex-1 py-3 bg-white/10 text-white rounded-xl font-bold hover:bg-white/20 transition-all text-center">
            إلغاء
          </router-link>
        </div>
      </div>
...
const saveChanges = async () => {
  ...
  try {
    ...
    await router.push({ name: 'visa.show', params: { id: route.params.id } });
  } catch (error) {
...
onMounted(async () => {
  ...
  if (booking.value) {
    form.value.status = booking.value.status || 'draft';
    ...
    form.value.service_fee = booking.value.pricing?.service_fee ?? 0;
    const vd = booking.value.visa_detail || {};
    form.value.visa_details = {
      visa_type: vd.visa_type || 'tourist',
      country: vd.country || '',
      visa_duration_id: vd.duration_row?.id || null,
      entry_type: vd.entry_type || 'single',
      visa_agent_id: vd.agent?.id || null,
      visa_number: vd.visa_number || '',
      validity_from: vd.validity_from || null,
      validity_to: vd.validity_to || null,
    };
  }
});
</script>
