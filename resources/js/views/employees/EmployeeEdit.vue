  ...
  <div class="space-y-8 animate-in fade-in duration-700">
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        ...
        <p class="text-text-muted mt-1">تعديل بيانات الموظف #{{ id }}</p>
      </div>
    </div>
    ...
    <div v-else-if="employee" class="bg-card-bg border border-white/10 rounded-2xl p-8">
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div class="flex gap-4 pt-4">
          ...
          >
            <CheckCircle v-if="!store.loading.update" class="w-4 h-4" />
    ...
    <div v-else class="text-center py-20">
      <div class="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center mx-auto mb-4">
        <UserX class="w-8 h-8 text-white/10" />
      </div>
      <p class="text-text-muted">الموظف غير موجود</p>
      <router-link
        to="/employees"
        class="inline-block mt-4 px-6 py-2 bg-gold hover:bg-gold/90 text-black rounded-xl font-bold text-sm transition-all"
      >
        العودة للموظفين
      </router-link>
    </div>
    ...
    <!-- Info Box -->
    <div v-if="employee" class="p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl">
      <div class="flex items-start gap-3">
        <Info class="w-5 h-5 text-blue-500 flex-shrink-0 mt-0.5" />
        <div class="text-sm">
          <p class="font-semibold text-blue-500 mb-1">معلومات مهمة</p>
          <p class="text-text-muted">
            سيتم تحديث بيانات الموظف فوراً بعد الحفظ. إذا قمت بتغيير البريد الإلكتروني، سيتم إرسال بريد إلكتروني جديد للموظف.
          </p>
        </div>
      </div>
    </div>
...
const handleSubmit = async () => {
  try {
    await store.updateEmployee(Number(id), form.value);
    store.addToast('تم تحديث بيانات الموظف بنجاح');
    router.push(`/employees/${id}`);
  } catch (error) {
    store.addToast('فشل تحديث بيانات الموظف', 'error');
  }
};
...
onMounted(async () => {
  ...
  if (employee.value) {
    form.value = {
      ...
      is_active: employee.value.is_active ?? true,
    };
  }
});
</script>
