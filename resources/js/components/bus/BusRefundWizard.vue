    ...
    <div class="p-6 bg-white/5 border-b border-white/10 flex flex-col md:flex-row items-center justify-between gap-4">
      <div>
        <h3 class="text-xl font-bold text-white flex items-center gap-2">
          ...
          نظام استرجاع حجوزات الباصات
        </h3>
        <span class="text-xs text-muted block mt-1">تطبيق العزل المحاسبي التام والتدقيق المالي الصارم (للموردين)</span>
      </div>
    ...
    <div class="p-8 relative min-h-[420px] flex flex-col justify-between">
      ...
      <div v-if="currentStep === 1" class="space-y-6">
        <div class="border-r-2 border-gold pr-4">
          ...
          <h4 class="text-lg font-bold text-white mt-1">تحديد حجز الباص المستهدف</h4>
        ...
        <div v-if="initialBooking" class="p-4 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-between">
          <div>
            ...
            <span class="text-base font-bold font-mono text-gold">#{{ initialBooking.id }}</span>
            <span class="text-xs text-white block mt-1">{{ initialBooking.customer?.full_name || 'عميل نقدي' }}</span>
          </div>
        ...
        <div v-else>
          <label class="block text-xs text-muted mb-2 uppercase tracking-widest font-mono">أدخل معرف الحجز</label>
          <input v-model="form.bookingId" type="number" placeholder="مثال: 123"
      ...
      <div v-if="currentStep === 2" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div class="p-4 rounded-2xl bg-white/5 border border-white/5">
            <span class="text-[10px] text-muted uppercase tracking-widest block font-mono">عملة الأصل</span>
          ...
          <div v-if="initialBooking" class="p-4 rounded-2xl bg-white/5 border border-white/5">
            <span class="text-[10px] text-muted uppercase tracking-widest block font-mono">عدد المقاعد</span>
            <span class="text-2xl font-extrabold font-mono text-white mt-1 block">{{ initialBooking.quantity }}</span>
          </div>
        </div>
        ...
        <div v-if="initialBooking" class="mt-4 p-4 rounded-2xl bg-white/5 border border-white/10 space-y-3">
          <h5 class="text-xs font-bold text-gold flex items-center gap-2">
            <span>ℹ️</span> تفاصيل المورد والرحلة
          </h5>
          
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs font-mono">
            <div>
              <span class="text-muted block">شركة الباص (المورد):</span>
              <span class="text-white font-bold">{{ initialBooking.inventory?.company?.name || 'غير محدد' }}</span>
            </div>
            <div>
              <span class="text-muted block">خط السير:</span>
              <span class="text-white font-bold">{{ initialBooking.inventory?.route || 'غير محدد' }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="currentStep === 3" class="space-y-6">
        <div class="border-r-2 border-gold pr-4">
          ...
          <h4 class="text-lg font-bold text-white mt-1">غرامة الإلغاء وصافي الاسترداد</h4>
          <p class="text-xs text-muted mt-0.5">حدد الرسوم المفروضة لحساب الصافي القابل للاسترداد.</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
          <div>
            <label class="block text-xs text-muted mb-2 uppercase tracking-widest font-mono">غرامة الإلغاء ({{ form.originalCurrency }})</label>
          ...
          <div class="p-6 rounded-2xl bg-gold/5 border border-gold/20 flex flex-col justify-center items-center text-center">
            <span class="text-xs text-gold font-mono uppercase tracking-widest">صافي الاسترجاع</span>
      ...
      <div v-if="currentStep === 4" class="space-y-6">
        <div class="border-r-2 border-gold pr-4">
          ...
          <p class="text-xs text-muted mt-0.5">اختر ما إذا كنت تريد الاحتفاظ بالرصيد لدى شركة الباص أو استرداد القيمة لخزينة الشركة.</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div :class="[
            ...
            form.destination === 'company_credit' ? 'bg-white/10 border-gold shadow-md' : 'bg-white/5 border-white/5 hover:border-white/20'
          ]" @click="form.destination = 'company_credit'">
            <div>
              <div class="flex items-center justify-between mb-3">
                <span class="text-xs font-mono text-gold font-bold uppercase tracking-wider">المسار أ</span>
                <span v-if="form.destination === 'company_credit'" class="text-gold">●</span>
              </div>
              <h5 class="text-base font-bold text-white">رصيد لدى شركة الباص (تسوية مورد)</h5>
              <p class="text-xs text-muted mt-2 leading-relaxed">
                يتم تقليل المديونية لشركة الباص أو زيادة الرصيد الدائن لديهم. يحافظ على السيولة النقدية.
              </p>
            </div>
          </div>
          ...
          <div :class="[
            ...
            <div>
              ...
              <p class="text-xs text-muted mt-2 leading-relaxed">
                استرداد نقدي فعلي من المورد إلى الخزينة. يتطلب اختيار خزينة مطابقة للعملة.
              </p>
            </div>
          </div>
        </div>
      ...
      <div v-if="currentStep === 5" class="space-y-6">
        <div class="border-r-2 border-gold pr-4">
          ...
          <p class="text-xs text-muted mt-0.5">تأكيد توافق العملات وتحديد الخزينة.</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label class="block text-xs text-muted mb-2 uppercase tracking-widest font-mono">عملة الاسترجاع</label>
          ...
          <div>
            <label class="block text-xs text-muted mb-2 uppercase tracking-widest font-mono">سعر الصرف</label>
        ...
        <div v-if="form.destination === 'agency_treasury'" class="pt-4 border-t border-white/5">
          <label class="block text-xs text-gold mb-3 uppercase tracking-widest font-mono">الخزائن المتوافقة المتاحة ({{ form.refundCurrency }})</label>
          
          <div v-if="filteredTreasuries.length === 0" class="p-4 rounded-xl bg-error/10 border border-error/20 text-center">
            <p class="text-xs text-error font-mono">لم يُعثر على خزائن مطابقة للعملة: {{ form.refundCurrency }}.</p>
          </div>

          <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-4">
             <div v-for="t in filteredTreasuries" :key="t.id" 
                  @click="form.treasuryId = t.id"
                  :class="['p-4 rounded-xl border-2 cursor-pointer transition-all', form.treasuryId === t.id ? 'bg-gold/10 border-gold' : 'bg-white/5 border-white/5']">
                <div class="flex justify-between items-center">
                  <span class="text-white font-bold">{{ t.name }}</span>
                  <span class="text-gold font-mono">{{ Number(t.current_balance).toLocaleString() }}</span>
                </div>
                <span class="text-[10px] text-muted">{{ t.currency }}</span>
             </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="currentStep === 6" class="space-y-6">
        <div class="border-r-2 border-gold pr-4">
          ...
          <h4 class="text-lg font-bold text-white mt-1">المراجعة النهائية</h4>
          <p class="text-xs text-muted mt-0.5">يرجى مراجعة كافة التفاصيل قبل التنفيذ.</p>
        </div>
        ...
        <div class="p-6 rounded-2xl bg-white/5 border border-white/10 space-y-4 font-mono text-xs">
          <div class="flex justify-between pb-2 border-b border-white/5">
            <span class="text-muted">رقم الحجز المستهدف</span>
            <span class="text-gold font-bold">#{{ initialBooking?.id || form.bookingId }}</span>
          </div>

          <div class="flex justify-between pb-2 border-b border-white/5">
            <span class="text-muted">المبلغ الإجمالي</span>
            <span class="text-white">{{ Number(form.originalAmount).toLocaleString() }} {{ form.originalCurrency }}</span>
          </div>

          <div class="flex justify-between pb-2 border-b border-white/5">
            <span class="text-muted">غرامة الإلغاء</span>
          ...
          <div class="flex justify-between pt-1">
            <span class="text-muted">الوجهة:</span>
            <span class="text-white font-bold">{{ form.destination === 'company_credit' ? 'رصيد/تسوية مورد' : 'خزينة الشركة' }}</span>
          </div>
        </div>

        <div>
          <label class="block text-xs text-muted mb-2 uppercase tracking-widest font-mono">ملاحظات</label>
          <textarea v-model="form.notes" rows="2" class="w-full p-4 bg-input border border-white/10 rounded-2xl focus:border-gold outline-none text-xs font-mono text-white text-right"></textarea>
        </div>
      </div>
      ...
      <div v-if="successResult" class="absolute inset-0 bg-card z-40 p-8 flex flex-col items-center justify-center text-center">
        ...
        <h4 class="text-xl font-bold text-white">تمت معالجة الاسترجاع بنجاح</h4>
        <div class="mt-6 p-4 rounded-xl bg-white/5 border border-white/5 w-full max-w-sm font-mono text-xs text-right space-y-2">
          ...
          <div class="flex justify-between"><span class="text-muted">المسترد:</span><span class="text-success font-bold">{{ successResult.refund_amount }} {{ successResult.refund_currency }}</span></div>
        </div>
...
const loadingText = ref('جاري التحقق...');
...
const setupFromBooking = (b) => {
  ...
  form.value.originalCurrency = 'EGP';
  form.value.originalAmount = Number(b.total_price || 0);
  form.value.refundCurrency = 'EGP';
};

const fetchTreasuries = async () => {
  try {
    const data = await busStore.fetchRefundTreasuries();
    treasuries.value = data;
  } catch (err) {
    console.error(err);
  }
};
...
const submitRefund = async () => {
  loading.value = true;
  loadingText.value = 'جاري ترحيل القيود...';
  try {
    const payload = {
      bus_booking_id: form.value.bookingId,
    ...
    const createdRefund = await busStore.createRefund(payload);
    if (createdRefund?.id) {
      const processRes = await busStore.processRefund(createdRefund.id);
      successResult.value = processRes;
    }
  } catch (err) {
    alert(err.response?.data?.message || 'تعذرت معالجة الاسترجاع.');
  } finally {
    loading.value = false;
  }
};
