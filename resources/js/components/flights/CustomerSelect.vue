<template>
  <div class="relative">
    ...
    <div v-if="modelValue" class="p-4 bg-input rounded-xl border border-gold/30 flex justify-between items-center animate-in fade-in slide-in-from-top-2">
      <div>
        <h3 class="font-bold text-gold">{{ modelValue.name }}</h3>
        <p class="text-sm text-muted">{{ modelValue.phone }} | {{ modelValue.email }}</p>
      </div>
    ...
    <div v-else class="relative">
      ...
      <input
        ...
        class="w-full pl-10 pr-4 py-3 bg-input border border-white/10 rounded-xl focus:border-gold focus:ring-1 focus:ring-gold outline-none transition-all"
        @input="onSearch"
        @keydown="handleKeydown"
      />

      <!-- Dropdown -->
      <div v-if="showDropdown" class="absolute z-50 w-full mt-2 bg-card border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div v-else class="max-h-60 overflow-y-auto">
          <button
            ...
            :class="['w-full p-3 text-left hover:bg-white/5 border-b border-white/5 last:border-0 transition-colors flex items-center gap-3', highlightedIndex === idx ? 'bg-white/5' : '']"
          >
            ...
            <div class="flex-1 min-w-0">
              <div class="font-medium truncate">{{ customer.name }}</div>
    ...
    <div v-if="showCreateModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
      <div class="bg-card w-full max-w-md border border-white/10 rounded-2xl p-6 shadow-2xl animate-in zoom-in-95 duration-300">
        <h2 class="text-xl font-bold mb-6 text-gold">إضافة عميل جديد</h2>
        <div class="space-y-4">
          <div>
            <label class="block text-sm text-muted mb-1">الاسم بالكامل*</label>
            <input v-model="newCustomer.full_name" type="text" class="w-full p-3 bg-input border border-white/10 rounded-xl focus:border-gold outline-none" />
          </div>
          <div>
            <label class="block text-sm text-muted mb-1">رقم الهاتف*</label>
            <input v-model="newCustomer.phone" type="text" class="w-full p-3 bg-input border border-white/10 rounded-xl focus:border-gold outline-none" />
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm text-muted mb-1">الرقم القومي</label>
              <input v-model="newCustomer.national_id" type="text" maxlength="14" class="w-full p-3 bg-input border border-white/10 rounded-xl focus:border-gold outline-none text-sm" />
            </div>
            <div>
              <label class="block text-sm text-muted mb-1">تاريخ الميلاد</label>
              <input v-model="newCustomer.date_of_birth" type="date" class="w-full p-3 bg-input border border-white/10 rounded-xl focus:border-gold outline-none text-sm" />
            </div>
          </div>
        </div>
...
const saveCustomer = async () => {
  if (!newCustomer.value.full_name || !newCustomer.value.phone) return;
  ...
  try {
    ...
    newCustomer.value = { full_name: '', phone: '', national_id: '', date_of_birth: '', type: props.type };
