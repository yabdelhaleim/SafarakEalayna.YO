...
/**
 * FULL DIAGNOSTIC for Account #7 (خزينة حورس باص) — combines categorization + deep dive on suspicious entries
 * ────────────────────────────────────────────────────────────────────────────────────────────────
 * Usage on server:
 *   cd /var/www/safarakealayna
 *   php _diag_account7_full.php
 */
...
$summary = [];
$suspicious = [];   // collect IDs of entries to deep-dive
foreach ($rows as $r) {
    $t = $r->type ?? 'OPEN';
    $m = $r->module ?? '-';
    ...
    if ($t === 'OPEN') {
        ...
        $cat = 'GENERAL_NO_NOTE';
        $suspicious[] = $r->id;       // collect for deep-dive
    } elseif ($m === 'general') {
    ...
    if (! isset($summary[$cat])) {
        $summary[$cat] = [
            ...
            'count_debit' => 0, 'count_credit' => 0, 'examples' => [],
        ];
    }
    $summary[$cat]['count']++;
...
// =====================================================================================
// PHASE 2: Deep dive on suspicious entries (no notes + module=general)
// =====================================================================================
if (empty($suspicious)) {
    echo PHP_EOL.str_repeat('═', 100).PHP_EOL;
    echo ' ✅ NO SUSPICIOUS ENTRIES — All entries have notes!'.PHP_EOL;
    echo str_repeat('═', 100).PHP_EOL;
    exit(0);
}
...
echo str_repeat('═', 100).PHP_EOL;
echo ' PHASE 2 — DEEP DIVE ON '.count($suspicious).' SUSPICIOUS ENTRIES (no notes + module=general)'.PHP_EOL;
...
foreach ($suspicious as $entryId) {
    ...
    try {
        ...
        if ($rev->isEmpty()) {
            echo '  ℹ️  No reversal (no other entries on this transaction_id)'.PHP_EOL;
        } else {
            foreach ($rev as $r) {
...
echo str_repeat('═', 100).PHP_EOL;
echo " DONE — Phase 1 + Phase 2 complete for Account #$accountId ({$account->name})".PHP_EOL;
echo str_repeat('═', 100).PHP_EOL;
