    ...
    protected function getStats(): array
    {
        return [
            Stat::make('إجمالي المعاملات', \App\Models\Online\OnlineTransaction::count())
                ...
                ->color('info'),
            Stat::make('إجمالي الأرباح', number_format(\App\Models\Online\OnlineTransaction::sum('profit') ?? 0, 2).' ج.م')
                ->description('صافي الربح من الخدمات الإلكترونية')
                ->descriptionIcon('heroicon-m-arrow-trending-up')
                ->color('success'),
            Stat::make('المبالغ المحصلة', number_format(\App\Models\Online\OnlineTransaction::sum('selling_price') ?? 0, 2).' ج.م')
