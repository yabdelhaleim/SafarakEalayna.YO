...
class FlightSystemsBalancesPage extends Page
{
    ...
    protected static function accountOptionLabel(Account $a): string
    {
        ...
        $typeLabel = match ($typeVal) {            AccountType::Cashbox->value => 'نقدي / درج',            AccountType::Bank->value => 'خزينة عامة',            default => $typeVal,        };        return "{$a->name} — {$typeLabel} — {$bal} {$cur}";    }    protected function getHeaderActions(): array    {        $systemOptions = fn (): array => FlightSystem::query()            ->where('is_active', true)            ->orderBy('name')            ->get()            ->mapWithKeys(fn (FlightSystem $s) => [                $s->id => "{$s->name} ({$s->code}) — {$s->currency}",            ])            ->all();        return [            Action::make('rechargeFlightSystem')                ->label('شحن رصيد نظام')                ->icon('heroicon-o-arrow-trending-up')                ->color('success')                ->modalHeading('إعادة شحن رصيد نظام حجز')                ->modalDescription('يُخصم المبلغ من حساب تحصيل (محفظة / بنك / خزينة) بنفس عملة النظام، ويُضاف لرصيد النظام مع قيد في الطيران.')                ->form([                    Select::make('flight_system_id')                        ->label('نظام الحجز')                        ->options($systemOptions())                        ->required()                        ->searchable()                        ->native(false)                        ->live()                        ->afterStateUpdated(fn (Set $set) => $set('from_account_id', null)),                    Select::make('from_account_id')                        ->label('من حساب (محفظة / بنك / خزينة)')                        ->options(function (Get $get): array {                            $sid = $get('flight_system_id');                            if (! $sid) {                                return [];                            }                            $system = FlightSystem::query()->find((int) $sid);                            if (! $system) {                                return [];                            }                            $types = [AccountType::Cashbox->value, AccountType::Wallet->value, AccountType::Bank->value];                            return Account::query()                                ->where('is_active', true)                                ->where('module_type', 'flights')                                ->whereIn('type', $types)                                ->where('currency', $system->currency)                                ->orderBy('type')                                ->orderBy('name')                                ->get()                                ->mapWithKeys(fn (Account $a) => [$a->id => self::accountOptionLabel($a)])                                ->all();
                        })
                        ->required()
                        ->searchable()
                        ->native(false)
