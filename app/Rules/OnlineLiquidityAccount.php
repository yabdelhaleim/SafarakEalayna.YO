...
/**
 * Validates that the selected account is a usable Online-module liquidity account.
 *
 * Phase 5 (Account Unification) — designed broadened from the start:
 *
 *  1. Strict per-module: `module_type='online'` OR `module='online'`
 ...
 *  - Other office modules (`module_type='bus'`, 'fawry', 'wallet_transfer')
 ...
 */
class OnlineLiquidityAccount implements ValidationRule
{
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        ...
        if (! self::belongsToOnlineModule($account)) {
            $fail('يجب أن يكون الحساب تابعاً لموديول الخدمات الإلكترونية أو خزينة قسم المكتب الموحّدة.');
        ...
        if (! in_array($type, AccountModuleContract::LIQUIDITY_TYPES, true)) {
            $fail('يجب اختيار حساب سيولة (خزينة / بنك / محفظة) تابع للخدمات الإلكترونية أو قسم المكتب.');
    ...
    /**
     * True if the account can be used as an Online-module liquidity vault.
     *
     * Acceptance matrix:
     *  ┌─────────────────────────────┬───────┐
     ...
     *  └─────────────────────────────┴───────┘
     */
    public static function belongsToOnlineModule(Account $account): bool
    {
        ...
        if ($moduleType === 'online' || $module === 'online') {
            return true;
        }
