<?php

namespace App\Http\Resources\Finance;

use Illuminate\Http\Request;
use Illuminate\Http\Resources\Json\JsonResource;

class AccountEntryResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        $transaction = $this->transaction;
        $related = $transaction?->related;
        
        $reference_id = null;
        $entity_name = null;
        $process_type = $transaction?->module?->label() ?? 'نظامي';
        $status = null;
        $provider_name = null;

        // Extract polymorphic data
        if ($related instanceof \App\Models\FlightBooking) {
            $reference_id = $related->booking_reference ?: $related->pnr;
            $entity_name = $related->customer?->full_name ?: ($related->passengers?->first() ? ($related->passengers->first()->first_name . ' ' . $related->passengers->first()->last_name) : null);
            $process_type = 'حجز طيران';
        } elseif ($related instanceof \App\Models\Bus\BusBooking) {
            $reference_id = $related->booking_number;
            $entity_name = $related->passenger_name;
            $process_type = 'حجز باص';
            $provider_name = $related->company?->name;
        } elseif ($related instanceof \App\Models\Online\OnlineTransaction) {
            $reference_id = $related->reference_number;
            $entity_name = $related->customer_name;
            $process_type = $related->serviceType?->name ?: 'خدمة أون لاين';
            $status = $related->status?->value;
            $provider_name = $related->provider?->name;
        } elseif ($related instanceof \App\Models\Transfer) {
            $process_type = 'تحويل مالي';
            $entity_name = $transaction->from_account_id == $this->account_id 
                ? ($transaction->toAccount?->name) 
                : ($transaction->fromAccount?->name);
        } elseif ($related instanceof \App\Models\VisaBooking) {
            $reference_id = $related->id;
            $entity_name = $related->customer?->full_name;
            $process_type = 'تأشيرة';
        } elseif ($related instanceof \App\Models\HajjUmraBooking) {
            $reference_id = $related->booking_number;
            $entity_name = $related->customer?->full_name;
            $process_type = 'حج وعمرة';
        }

        return [
            'id' => $this->id,
            'account_id' => $this->account_id,
            'transaction_id' => $this->transaction_id,
            'type' => $transaction?->type?->value ?? ($this->credit > 0 ? 'credit' : 'debit'),
            'type_label' => $transaction?->type?->label() ?? ($this->credit > 0 ? 'إيداع/رصيد' : 'صرف'),
            'module_label' => $transaction?->module?->label() ?? 'نظامي',
            'description' => $this->notes ?: ($transaction?->notes ?: 'معاملة مالية'),
            'user_name' => $transaction?->createdBy?->name ?? 'النظام',
            'debit' => (float) $this->debit,
            'credit' => (float) $this->credit,
            'balance_after' => (float) $this->balance_after,
            'created_at' => $this->created_at?->format('Y-m-d H:i:s'),
            'date_human' => $this->created_at?->translatedFormat('j F Y, g:i a'),
            
            // Enhanced Fields
            'reference_id' => $reference_id,
            'entity_name' => $entity_name,
            'process_type' => $process_type,
            'status' => $status,
            'provider_name' => $provider_name,
            'payment_method' => $transaction?->posting_channel ?: 'نقدي',
        ];
    }
}
