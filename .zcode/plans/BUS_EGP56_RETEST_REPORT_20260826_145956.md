...
- Ledger invariant: **1 imbalanced accounts**
- Non-EGP Bus rows: **0 (clean)**
...
| FM-14 | Overpayment rejected | PASS | 3 | overpay_reject=YES pmt_delta=0 |
...
| FM-G09-RG | Reject HTTP non-EGP booking at payBooking | RG-PASS | 3 | Bus module is EGP-only (EGP). Rejected non-EGP currency (boo |
...
| FM-23 | Cancel after pay-debt BLOCKED | PASS | 3 | cancel_blocked=YES supplier_bal_booking=-80.00 supplier_bal_pre_cancel=100.00 msg=فشل إلغاء الحجز: لا يمكن إلغاء هذ� |
...
| FM-33 | Cash inventory delete reverses expense | PASS | 5 | entries=2→4 cashbox_Δ_create=-400.00 cashbox_Δ_delete=0.00 |
...
| FM-G36-RG | Reject USD booking→EGP wallet (HTTP layer) | RG-PASS | 3 | Bus module is EGP-only (EGP). Rejected non-EGP currency (boo |
| FM-G37-RG | Reject USD booking→EGP cashbox FX (HTTP layer) | RG-PASS | 3 | Bus module is EGP-only (EGP). Rejected non-EGP currency (boo |
| FM-G38-RG | Reject SAR booking→EGP wallet (HTTP layer) | RG-PASS | 3 | Bus module is EGP-only (EGP). Rejected non-EGP currency (boo |
| FM-G39-RG | Reject SAR booking→EGP cashbox FX (HTTP layer) | RG-PASS | 2 | Bus module is EGP-only (EGP). Rejected non-EGP currency (boo |
| FM-G40-RG | Reject KWD booking high-rate (HTTP layer) | RG-PASS | 3 | Bus module is EGP-only (EGP). Rejected non-EGP currency (boo |
| FM-G41-RG | Reject multi-currency AR stacking | RG-PASS | 3 | Bus module is EGP-only (EGP). Rejected non-EGP currency (exi |
...
| FM-44 | Replay with different tuple (key wins) | PASS | 3 | same_key_diff_tuple=REPLAYED pmts=1 |
| FM-45 | Replay with different amount (key wins) | PASS | 3 | same_key_diff_amount=REPLAYED pmts=1 |
...
| FM-54 | Direct status write after cancel (operational) | PASS | 1 | status=pending |
...
| FM-67 | Customer AR net = sale - payment - reversal | PASS | 3 | ar_start=2650.00 after_pay=2450.00 after_cancel=2450.00 expected_Δ=-200.00 |
